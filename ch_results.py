...
'''
def VI_transform_by_class(model='hfl/chinese-bert-wwm'):
    """
    Return the v-info for every input transformation.
    """
    load_data()
    table = {}
    transformations = ['chinese-bert-wwm_test22', 'chinese-bert-wwm_test_shuffled', 'chinese-bert-wwm_test_hypothesis_only', 'chinese-bert-wwm_test_premise_only', 'chinese-bert-wwm_test_overlap' ]
...
def VI_transform_by_class(model='chinese-bert-wwm'): 
    """
    Return the v-info for every input transformation.
    """
    load_data()
    ...
    for i in [0, 1, 2]:
        ...
        for j in transformations:
            # 安全检查：确保数据集中存在PVI列
            if 'PVI' in DATA[model][j].columns:
                # 获取指定类别（label == i）的PVI平均值
                table[i][j] = DATA[model][j][DATA[model][j]['label'] == i]['PVI'].mean()
            else:
                table[i][j] = None  # 如果PVI列不存在，返回None或者其他处理方式
...
def breakdown_by_agreement(suffix="std"):
    ...
    for row in open('data/ocnli_test_std.csv'):
        row = json.loads(row)
